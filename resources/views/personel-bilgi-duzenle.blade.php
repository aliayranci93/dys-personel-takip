                              ...
                              <div class="row">
                                 <div class="row mb-5">
                                    ...
                                    <div class="col-lg-6 text-right">
                                       <button type="button" data-target="#egitim_durumu" data-id="{{$personelBilgi->id}}" data-toggle="modal" data-bs-target="#modal-3" data-bs-toggle="modal"  class="btn btn-light" id ="egitim_durum_add_btn"><i class="icon-plus"></i> Add Record</button>
...
<!-- end: Page Content -->
<script src="{{env('APP_URL')}}/plugins/datatables/datatables.min.js"></script>
<script>
...
debugger;
   var table = $('#datatable2').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('egitim-durumu-getir') }}",
        columns: [
            {data: 'DT_RowIndex', name: 'DT_RowIndex'},
            {data: 'ogrenim_durumu_id', name: 'ogrenim_durumu_id'},
            {data: 'okulun_adi_bolumu', name: 'okulun_adi_bolumu'},
            {data: 'mezuniyet_yili', name: 'mezuniyet_yili'},
            {data: 'action', name: 'action', orderable: false, searchable: false},
        ]
    });

   $('#egitim_durumu_button').click(function (e) {
      debugger;
        e.preventDefault();
        $(this).html('Sending..');
    
        $.ajax({
            data: $('#egitim_durumu').serialize(),
            url: "{{route('egitim-durumu-kaydet')}}",
            type: "POST",
            dataType: 'json',
            success: function (data) {
               debugger;
                $('#egitim_durumu').trigger("reset");
                $('#modal-3').modal('hide');
                table.draw();
            },
            error: function (data) {
